...
// Menu items
enum MenuItems_Custom
{
    MENUITEM_CUSTOM_BATTLESPEED,
    MENUITEM_CUSTOM_DIFFICULTY,
    MENUITEM_CUSTOM_FOLLOWMONS,
...
{
    [MENUITEM_CUSTOM_BATTLESPEED]    = {DrawChoices_BattleSpeed,  ProcessInput_Options_Four},
    [MENUITEM_CUSTOM_DIFFICULTY]     = {DrawChoices_Difficulty,   ProcessInput_Options_Three},
    [MENUITEM_CUSTOM_FOLLOWMONS]     = {DrawChoices_FollowMons,   ProcessInput_Options_Two},
...
static const u8 *const sOptionsMenuItemsNamesCustom[MENUITEM_CUSTOM_COUNT] =
{
    [MENUITEM_CUSTOM_BATTLESPEED]    = sText_Battlespeed,
    [MENUITEM_CUSTOM_DIFFICULTY]     = sText_Difficulty,
    [MENUITEM_CUSTOM_FOLLOWMONS]     = sText_FollowMons,
    [MENUITEM_CUSTOM_AUTORUN]        = sText_Autorun,
    [MENUITEM_CUSTOM_FONT]           = sText_Font,
    [MENUITEM_CUSTOM_CANCEL]         = sText_Cancel,
};
...
static const u8 *const OptionTextRight(u8 menuItem)
{
    ...
    {
        case MENU_CUSTOM:
            return sOptionsMenuItemsNamesCustom[menuItem];
        case MENU_VANILLA:
...
static bool8 CheckConditions(int selection)
{
    switch (sOptions->submenu)
    {
        case MENU_CUSTOM:
            switch(selection)
            {
                case MENUITEM_CUSTOM_BATTLESPEED:     return TRUE;
                case MENUITEM_CUSTOM_DIFFICULTY:      return TRUE;
                case MENUITEM_CUSTOM_FOLLOWMONS:      return TRUE;
...
static const u8 sText_Desc_DifficultyEasy[]     = _("Easy difficulty. Enemies are not EV trained.\nItems allowed.");
static const u8 sText_Desc_DifficultyNormal[]   = _("Normal difficulty. Balanced battles.\nItems allowed.");
static const u8 sText_Desc_DifficultyHard[]     = _("Hard difficulty. Battle style forced\nto SET. No items and optimised teams.");
...
static const u8 sText_Desc_Font[]               = _("Choose the font used in the game.\nFireRed/LeafGreen, Emerald.");
...
static const u8 sText_Desc_BattleStyle_Default[] = _("Battle style forced to SET only.\nOnly on HARD difficulty.");
...
static const u8 *const sOptionsMenuItemDescriptionsCustom[MENUITEM_CUSTOM_COUNT][4] =
{
    [MENUITEM_CUSTOM_BATTLESPEED]    = {sText_Desc_BattleSpeed,          sText_Desc_BattleSpeed,     sText_Desc_BattleSpeed,  sText_Desc_BattleSpeed},
    [MENUITEM_CUSTOM_DIFFICULTY]     = {sText_Desc_DifficultyEasy,       sText_Desc_DifficultyNormal, sText_Desc_DifficultyHard, sText_Empty},
    [MENUITEM_CUSTOM_FOLLOWMONS]     = {sText_Desc_FollowMon_On,         sText_Desc_FollowMon_Off,   sText_Empty,             sText_Empty},
    [MENUITEM_CUSTOM_AUTORUN]        = {sText_Desc_Autorun_On,           sText_Desc_Autorun_Off,     sText_Empty,             sText_Empty},
    [MENUITEM_CUSTOM_FONT]           = {sText_Desc_Font,                 sText_Desc_Font,            sText_Empty,         sText_Empty},
...
static const u8 *const OptionTextDescription(void)
{
    ...
    case MENU_CUSTOM:
        ...
        return sOptionsMenuItemDescriptionsCustom[menuItem][selection];
    case MENU_VANILLA:
...
static void DrawTopBarText(void)
{
    ...
    {
        case MENU_CUSTOM:
            AddTextPrinterParameterized3(WIN_TOPBAR, FONT_SMALL, 40, 1, color, 0, sText_TopBar_Custom);
            break;
        case MENU_VANILLA:
...
void CB2_InitOptionPlusMenu(void)
{
    ...
    case 5:
        ...
        break;
    case 6:
        sOptions->sel_van[MENUITEM_VANILLA_TEXTSPEED]   = gSaveBlock2Ptr->optionsTextSpeed;
        sOptions->sel_custom[MENUITEM_CUSTOM_BATTLESPEED]     = gSaveBlock2Ptr->optionsBattleSpeed;
        sOptions->sel_custom[MENUITEM_CUSTOM_DIFFICULTY]      = gSaveBlock2Ptr->optionsDifficulty;
        sOptions->sel_custom[MENUITEM_CUSTOM_FOLLOWMONS]      = gSaveBlock2Ptr->optionsFollowMonsOff;
...
//        sOptions->sel_van[MENUITEM_VANILLA_FRAMETYPE]    = gSaveBlock2Ptr->optionsWindowFrameType;
    ...
    case 10:
        ...
        sOptions->arrowTaskId = AddScrollIndicatorArrowPairParameterized(SCROLL_ARROW_UP, 240 / 2, 20, 110, MENUITEM_CUSTOM_COUNT - 1, 110, 110, 0);
...
static void Task_OptionsMenuSave(u8 taskId)
{
    //write in saveblock
    //custom settings
    gSaveBlock2Ptr->optionsBattleSpeed      = sOptions->sel_custom[MENUITEM_CUSTOM_BATTLESPEED];
    gSaveBlock2Ptr->optionsDifficulty       = sOptions->sel_custom[MENUITEM_CUSTOM_DIFFICULTY];
    gSaveBlock2Ptr->optionsFollowMonsOff    = sOptions->sel_custom[MENUITEM_CUSTOM_FOLLOWMONS];
    ...
    //vanilla settings
    gSaveBlock2Ptr->optionsTextSpeed        = sOptions->sel_van[MENUITEM_VANILLA_TEXTSPEED];
    ...
    //set flags/VARs
    VarSet(VAR_BATTLESPEED, sOptions->sel_custom[MENUITEM_CUSTOM_BATTLESPEED]);
    VarSet(VAR_DIFFICULTY, sOptions->sel_custom[MENUITEM_CUSTOM_DIFFICULTY]);

    if (sOptions->sel_custom[MENUITEM_CUSTOM_FOLLOWMONS] == TRUE)
...
static const u8 sText_NormalDifficulty[]    = _("NORMAL");
static const u8 sText_Hard[]                = _("HARD");
static const u8 sText_FontTypeFRLG[] = _("FR/LG");
static const u8 sText_FontTypeEMRLD[] = _("EMRLD");
...
static void DrawChoices_TextSpeed(int selection, int y)
{
    ...
    styles[selection] = 1;
    xMid = GetMiddleX(sText_Slow, sText_Mid, sText_Fast);

    DrawOptionsMenuChoice(sText_Slow, 104, y, styles[0], active);
    DrawOptionsMenuChoice(sText_Mid, xMid, y, styles[1], active);
    DrawOptionsMenuChoice(sText_Fast, GetStringRightAlignXOffset(1, sText_Fast, 198), y, styles[2], active);
    //DrawOptionsMenuChoice(sText_Instant, GetStringRightAlignXOffset(1, sText_Instant, 198), y, styles[2], active);
}

static void DrawChoices_BattleSpeed(int selection, int y)
{
    bool8 active = CheckConditions(MENUITEM_CUSTOM_BATTLESPEED);
    u8 styles[4] = {0};
    int xSpacer;
...
static void DrawChoices_Difficulty(int selection, int y)
{
    ...
    styles[selection] = 1;
    xMid = GetMiddleX(sText_Easy, sText_NormalDifficulty, sText_Hard);

    DrawOptionsMenuChoice(sText_Easy, 104, y, styles[0], active);
    DrawOptionsMenuChoice(sText_NormalDifficulty, xMid, y, styles[1], active);
    DrawOptionsMenuChoice(sText_Hard, GetStringRightAlignXOffset(1, sText_Hard, 198), y, styles[2], active);
}

static void DrawChoices_FollowMons(int selection, int y)
{
    bool8 active = CheckConditions(MENUITEM_CUSTOM_FOLLOWMONS);
    u8 styles[2] = {0};
    styles[selection] = 1;
...
static void DrawChoices_Font(int selection, int y)
{
    ...
    DrawOptionsMenuChoice(sText_FontTypeEMRLD, 104, y, styles[0], active);
    DrawOptionsMenuChoice(sText_FontTypeFRLG, GetStringRightAlignXOffset(FONT_NORMAL, sText_FontTypeFRLG, 198), y, styles[1], active);
}
